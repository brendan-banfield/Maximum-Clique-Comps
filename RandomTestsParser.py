...
sizes = []
clique_sizes = []
with open("randomGraphsOutput.txt", "r") as f:
    ...
    while i < len(lines):
        ...
        Bron_Kerbosh_success = int(Bron_Kerbosh_info[16][:-1])
        

        # line 4
        Branch_and_Bound_info = lines[i + 3].split()
        Branch_and_Bound_time = float(Branch_and_Bound_info[5])
        Branch_and_Bound_success = int(Branch_and_Bound_info[16][:-1])
        
        if (Branch_and_Bound_success > 0 or Bron_Kerbosh_success  > 0):
...
for i in range(len(sizes)):
    if sizes[i] in sim_ann_dict:
        sim_ann_dict[sizes[i]].append(successes[2][i])
    else:
        sim_ann_dict[sizes[i]] = [successes[2][i]]
    if sizes[i] in gen_alg_dict:
        gen_alg_dict[sizes[i]].append(successes[3][i])
    else:
        gen_alg_dict[sizes[i]] = [successes[3][i]]
