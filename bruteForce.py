...
class Brute_Force_Solver:
    ...
    def run(self):
        # there are 2^|V|-1 nonempty subsets (permutations)
        for _ in range(int(pow(2, self.g.vertices))-1):
            self.next_permutation()
            # Could easily choose to ignore subsets smaller than the largest clique found so far, 
            # but that is an optimization and not in the true spirit of brute force.
            # So I'm not doing it.
